...
"""
def train_surrogate_random(current_vqe, n_samples=100, epochs=15):
    print("-----Training and Testing surrogate on random parameters-----")
    ## Generating random data.
    print("Generating training/validation data.")
    random_data = []
    for i in range(n_samples):
        params = np.random.uniform(0, 2 * np.pi, len(current_vqe.ansatz.symbols))
        random_data.append({"params": params, "energy": current_vqe.vqe_cost(params)})
    
    ## Training QML model (i.e., the surrogate)    
    print("Training the surrogate.")
    print("  - loading the data.")
    current_vqe.surrogate.load_data(random_data, current_vqe.ansatz)
    print("  - fitting the QML model.")
    history = current_vqe.surrogate.tfq_model.fit(x=current_vqe.surrogate.train_states, 
                                                    y=current_vqe.surrogate.train_labels,
                                                    batch_size=32,
                                                    epochs=epochs,
                                                    verbose=1,
                                                    validation_data=(current_vqe.surrogate.test_states, 
                                                                    current_vqe.surrogate.test_labels))
